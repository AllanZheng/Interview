...
var url = 'http://127.0.0.1:8000/sendSecretToMe';
var data = {
    ...
    req.on('end', () => {
      ...
      if (!status) {
        ...
        switch (url) {
          ...
          case '/create':
            // 创建task请求
            result = service.createTask(requestBody)
            back(res, result)
            break;
          case '/update':
            // 更新task请求
            result = service.updateTask(requestBody)
            back(res, result)
            break;
          case '/delete':
            // 删除task请求
            result = service.deleteTask(requestBody)
            back(res, result)
            break;
          case '/all':
            // 获取所有task请求
          ...
          case '/get':
            // 通过id获取单个task请求
            result = service.getTaskById(requestBody)
            back(res, result)
            break;
          default:
...
function back(res, result) {
  ...
  if (result != null) {
    ...
    res.end(JSON.stringify(new resp("200", "", result)));
  } else {
    res.writeHead(400, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify(new resp("400", "failed", result)));
  }
}
